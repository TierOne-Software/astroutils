 ...
 * assert that direction.  Pass "casper" to test the read-only
 * casper mode instead of the full lockdown.
 *
 * Limits applied before caph_enter() are consolidated and take effect
 * at enter time, so all behavioral checks run post-enter.
 *
 * Prints one line per check and exits non-zero on any failure, 77
 * when the runtime lacks Landlock/seccomp (e.g. qemu-user).
 */
...
main(int argc, char *argv[])
{
	...
	const char *sbx = getenv("ASTROUTILS_SANDBOX");

	/* the test runner sets the escape hatch on sandbox-less runtimes */
	if (sbx != NULL && strcmp(sbx, "NONE") == 0)
		stub = 1;
	int fd, fd2, rc;
	char buf[8];
	cap_rights_t rights;
	...
	if (casper) {
		...
		/* last: a successful exec replaces us; probe in a child */
		{
			pid_t pid = fork();
			int st = 0;
	...
	/* --- register all limits (applied at caph_enter) ------------- */

	/* write-only: read and lseek will be denied, write/fstat allowed */
	cap_rights_init(&rights, CAP_WRITE, CAP_FSTAT);
	rc = caph_rights_limit(fd, &rights);
	check("rights_limit write-only accepted", rc == 0 || stub);

	/* read-only: write will be denied, read allowed */
	cap_rights_init(&rights, CAP_READ, CAP_FSTAT);
	rc = caph_rights_limit(fd2, &rights);
	check("rights_limit read-only accepted", rc == 0 || stub);
	...
	/* a lookup-enabled directory fd (write(1)'s /dev pattern) */
	{
...

	{
		...
		check("unlisted ioctl denied", stub ? 1 :
		    (rc < 0 && errno == EPERM));
	}
	...
	/* last: a successful exec replaces us, so probe it in a child */
	{
		pid_t pid = fork();
		int st = 0;
		...
		check("execve after enter denied", stub ? 1 :
		    (pid > 0 && WIFEXITED(st) && WEXITSTATUS(st) == 42));
	}
